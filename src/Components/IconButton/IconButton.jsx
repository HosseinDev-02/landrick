export default function IconButton({ icon, link, onClick, className, animation }) {
    return (
        <a onClick={onClick} className={`h-9 w-9 rounded-md z-50 bg-primary text-white flex items-center justify-center fixed group shadow shadow-primary/30 ${className}`}
           href={`${link || '#'}`}>
